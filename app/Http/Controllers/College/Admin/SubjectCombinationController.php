...
use App\Models\college\Subject_combination;
...
class SubjectCombinationController extends Controller
{
    ...
    public function index($subID)
    {
        ...
        return view('college.admin.subject_combination.list',$data);
    }
    ...
    public function create($subID)
    {
        ...
        return view('college.admin.subject_combination.create',$data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($validate[0] == 1)
        {
            return redirect()->route('college.admin.addSubjectCombination',$subID)->with("error_message","Already exists !!");
        }
        $res = $model->saveData($model);

        $res = $model->saveData($model);
        return redirect()->route('college.admin.subject_combination_list',$subID)->with('message',"Data has been saved...!");
    }
    ...
    public function edit($subID,$id)
    {
        ...
        if(!empty($editData))
        {
            ...
            return view('college.admin.subject_combination.edit',$data);
        }
        else
        {
            abort(404);
    ...
    public function update(Request $request,$subID,$id)
    {
        ...
        if($validate[0] == 1 && !empty($chkID))
        {
            ...
            return Redirect::to('college/admin/subjects/combinations/edit/'.$subID.'/'.$id)->with("error_message","Already exists !!");
        }

        $model->save();
        return redirect()->route('college.admin.subject_combination_list',$subID)->with('message',"Data has been updated...!");
    }
