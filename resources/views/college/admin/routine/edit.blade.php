...
<div class="container-xxl flex-grow-1 container-p-y">
    <h4 class="py-3 mb-4"><span class="text-muted fw-light"><a href="{{ route('college.admin.routineList') }}">Routine List</a> /</span> {{ $title }}</h4>
    <div class="row mb-4">
        ...
        <div class="col-md">
            <div class="card">
                ...
                <div class="card-body">
                    <form action=" {{ route('college.admin.updateRoutine',$data->id) }}" method="POST" class="needs-validation" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="sem" id="sem" class="form-select">
                                ...
                                @if(!empty($semData))
                                    ...
                                    <option value="{{ $s->id }}" <?php echo $s->id == $data->semID?'selected':''?>>{{ $s->title }}</option>
                                    @endforeach
                        ...
                        <div class="mb-3">
                            ...
                            <select name="weekdays" id="weekdays" class="form-select">
                                ...
                                @if(!empty($weekdays))
                                    ...
                                    <option value="{{ $k }}" <?php echo $k == $data->weekID?'selected':''?>>{{ $v }}</option>
                                    @endforeach
                        ...
                        <div class="mb-3">
                            ...
                            <select name="times" id="times" class="form-select">
                                ...
                                @if(!empty($timeData))
                                    ...
                                    <option value="{{ $t->id }}" <?php echo $t->id == $data->timeID?'selected':''?>>{{ $t->start_time .' - '.$t->end_time }}</option>
                                    @endforeach
